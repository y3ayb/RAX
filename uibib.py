...
NEXT_PAD = 180          #Posição de onde mostra a proxima peça
...
def buildPreview (display):
    ...
    coins_blue.grid (row = 0, column = 0, padx=(0,COIN_PAD))
    next_blue = Label (score_frame, text = '0', font = SCORE_LABEL_FONT, fg = 'blue')
    next_blue.grid (row = 0, column = 1, padx=(0,COIN_PAD))
    arrow_blue = Label (score_frame, text = '0', font = SCORE_LABEL_FONT, fg = 'blue')
    arrow_blue.grid (row = 0, column = 2, padx=(0,NEXT_PAD))
    score_label = Label(score_frame, text = '', font = SCORE_FONT)
    score_label.grid(row = 0, column = 3)
    arrow_red = Label (score_frame, text = '0', font = SCORE_LABEL_FONT, fg = 'red')
    arrow_red.grid (row = 0, column = 4, padx=(NEXT_PAD,0))
    next_red = Label(score_frame, text = '0', font = SCORE_LABEL_FONT, fg = 'red')
    next_red.grid(row = 0, column = 5, padx=(COIN_PAD,0))
    coins_red = Label(score_frame, text = '0', font = SCORE_LABEL_FONT, fg = 'red')
    coins_red.grid(row = 0, column = 6, padx=(COIN_PAD,0))
    return score_label, coins_blue, coins_red, next_blue, next_red, arrow_blue, arrow_red
...
def drawGridCellsButton(display):
    ...
    display.arrow_blue.configure(image = display.arrow[blue_arrow])
    display.arrow_red.configure(image = display.arrow[red_arrow])
...
def gameOver (display, player):
    ...
    display.next_blue.configure(image = display.pieces['empty'])
    display.next_red.configure(image = display.pieces['empty'])
    display.arrow_blue.configure(image = display.pieces['empty'])
    display.arrow_red.configure(image = display.pieces['empty'])
